...
export function ContactForm() {
  const [formData, setFormData] = useState({
    ...
    phone: "",
    message: "",
    website_url: "", // Honeypot field for bot trapping
  });

  // Track initial form load timestamp for time-delta bot detection
  const [formTime] = useState<number>(() => Date.now());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (nameErr || emailErr || phoneErr || msgErr) {
      setErrorMessage(
        "Please fix the highlighted errors in the form before submitting."
      );
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
              For business enquiries, partnerships, solar quotes, or vendor
              registration.
            </p>
          </div>
          ...
          <p className="text-[11px] text-slate-400 text-center flex items-center justify-center gap-1.5 pt-1">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-500" /> Your
            information is 100% secure with GNX Power Solution.
          </p>
        </form>
      ) : (
        ...
        <div className="py-4 flex flex-col items-center text-center space-y-6 animate-in fade-in zoom-in-95 duration-500">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 mt-2 max-w-md mx-auto leading-relaxed">
              Your inquiry has been submitted successfully. Our solar solution
          ...
          <div className="w-full p-6 sm:p-7 rounded-3xl bg-gradient-to-br from-cyan-50/80 via-white to-[#e6f7fa] dark:from-slate-900 dark:via-slate-900 dark:to-slate-950 border border-cyan-500/25 shadow-xl space-y-4 text-left relative overflow-hidden">
            ...
            <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 leading-relaxed font-normal">
              Manage products like solar panels, battery inverters, and place
              requests to buy or sell as a vendor now.
            </p>
